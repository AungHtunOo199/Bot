...
BOT_TOKEN = '8904188776:AAEVnXdWj7JW88E_69_SvQt-abX5diQev_k'
GITHUB_TOKEN = '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'
REPO_OWNER = "AungHtunOo199"
REPO_NAME = "Bot"
ADMIN_ID = "1939066924"
##################
...
async def handle_key(message):
    # Local mode: /key ခေါ်သည်နှင့် တန်း approve လုပ်ပါသည်။
    approve[message.chat.id] = True
    user_data[message.chat.id] = {}
    await bot.reply_to(
        message,
        " Key အလုပ်လုပ်ပါပြီ။ /input ဖြင့် Session URL ထည့်ပါ။"
    )
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    if 'logonUrl' in response:
        ...
        # --- Plan နှင့် Time ကို ရနိုင်သမျှ Ruijie Key အကုန်စစ်ပြီး ဆွဲထုတ်ခြင်း ---
        plan_name = "Unknown"
        plan_time = "Unknown"

        try:
            parsed_resp = json.loads(response)
            if 'data' in parsed_resp and isinstance(parsed_resp['data'], dict):
                res_data = parsed_resp['data']
                
                # Plan Name ဖြစ်နိုင်သမျှ Key အကုန်စစ်မည်
                plan_name = str(res_data.get('planName') or 
                                res_data.get('packageName') or 
                                res_data.get('package') or 
                                res_data.get('plan') or 
                                res_data.get('name') or "Unknown")
                
                # Time / Duration ဖြစ်နိုင်သမျှ Key အကုန်စစ်မည်
                plan_time = str(res_data.get('duration') or 
                                res_data.get('time') or 
                                res_data.get('timeAmount') or 
                                res_data.get('lifespan') or "Unknown")
        except Exception:
            pass

        # --- ပုံစံချခြင်း ---
        formatted_code = f"💳 {code}\n   📋 Plan: {plan_name} | ⏳ Time: {plan_time}"
